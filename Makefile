# This Makefile is for the TOON extension to perl.
#
...
#     ABSTRACT => q[Token-Oriented Object Notation for Perl]
#     AUTHOR => [q[Dave Cross <dave@perlhacks.com>]]
#     BUILD_REQUIRES => {  }
#     CONFIGURE_REQUIRES => {  }
#     EXE_FILES => [q[bin/toon_pp]]
#     NAME => q[TOON]
#     PREREQ_PM => { Exporter=>q[0], Getopt::Long=>q[0], Scalar::Util=>q[0] }
#     TEST_REQUIRES => {  }
#     VERSION_FROM => q[lib/TOON.pm]
#     test => { TESTS=>q[t/*.t] }
...
manifypods : pure_all config  \
	...
	$(NOECHO) $(POD2MAN) --section=$(MAN3EXT) --perm_rw=$(PERM_RW) -u \
	  lib/TOON.pm $(INST_MAN3DIR)/TOON.$(MAN3EXT) 
...
EXE_FILES = bin/toon_pp

pure_all :: $(INST_SCRIPT)/toon_pp
	$(NOECHO) $(NOOP)
...
$(INST_SCRIPT)/toon_pp : bin/toon_pp $(FIRST_MAKEFILE) $(INST_SCRIPT)$(DFSEP).exists $(INST_BIN)$(DFSEP).exists
	$(NOECHO) $(RM_F) $(INST_SCRIPT)/toon_pp
	$(CP) bin/toon_pp $(INST_SCRIPT)/toon_pp
	$(FIXIN) $(INST_SCRIPT)/toon_pp
	-$(NOECHO) $(CHMOD) $(PERM_RWX) $(INST_SCRIPT)/toon_pp
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'abstract: '\''Token-Oriented Object Notation for Perl'\''' >> META_new.yml
	$(NOECHO) $(ECHO) 'author:' >> META_new.yml
	$(NOECHO) $(ECHO) '  - '\''Dave Cross <dave@perlhacks.com>'\''' >> META_new.yml
	...
	$(NOECHO) $(ECHO) 'name: TOON' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '   "abstract" : "Token-Oriented Object Notation for Perl",' >> META_new.json
	$(NOECHO) $(ECHO) '   "author" : [' >> META_new.json
	$(NOECHO) $(ECHO) '      "Dave Cross <dave@perlhacks.com>"' >> META_new.json
	$(NOECHO) $(ECHO) '   ],' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "TOON",' >> META_new.json
...
ppd :
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="TOON" VERSION="0.0.1">' > TOON.ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT>Token-Oriented Object Notation for Perl</ABSTRACT>' >> TOON.ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Dave Cross &lt;dave@perlhacks.com&gt;</AUTHOR>' >> TOON.ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> TOON.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Exporter::" />' >> TOON.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Getopt::Long" />' >> TOON.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Scalar::Util" />' >> TOON.ppd
	$(NOECHO) $(ECHO) '        <ARCHITECTURE NAME="x86_64-linux-gnu-thread-multi-5.38" />' >> TOON.ppd
	$(NOECHO) $(ECHO) '        <CODEBASE HREF="" />' >> TOON.ppd
	$(NOECHO) $(ECHO) '    </IMPLEMENTATION>' >> TOON.ppd
	$(NOECHO) $(ECHO) '</SOFTPKG>' >> TOON.ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  'lib/TOON.pm' 'blib/lib/TOON.pm' \
	  'lib/TOON/Error.pm' 'blib/lib/TOON/Error.pm' \
	  'lib/TOON/PP.pm' 'blib/lib/TOON/PP.pm' 
